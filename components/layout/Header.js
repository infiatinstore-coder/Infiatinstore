...
export default function Header({ variant = 'default', title = '', showBack = false }) {
    ...
    if (variant === 'default') {
        return (
            <header className={cn(
                "sticky top-0 z-50 w-full transition-all duration-200 bg-gradient-to-r from-secondary-600 to-secondary-400",
                ...
                <div className="container-app flex items-center gap-4 px-4 md:px-0 justify-between">
                    ...
                    <div className="flex-1 max-w-2xl hidden md:block">
                        <form onSubmit={handleSearch} className="bg-white p-1 rounded-sm shadow-sm flex items-center">
                            ...
                            <button
                                type="submit"
                                className="bg-secondary-500 px-6 py-2 rounded-sm hover:opacity-90 transition-opacity"
                            >
                    ...
                    <div className="flex items-center gap-3 md:gap-6">
                        ...
                        >
                            ...
                            {itemsCount > 0 && (
                                <span className="absolute -top-1 -right-1 bg-white text-primary-600 text-[10px] font-bold px-1.5 py-0.5 rounded-full border border-primary-500">
                                    {itemsCount}
                                </span>
                            )}
                        </button>
                ...
                {isMenuOpen && (
                    <div className="fixed inset-0 z-[100] md:hidden">
                        ...
                        <div className="absolute top-0 left-0 w-[80%] max-w-[300px] h-full bg-white shadow-xl flex flex-col">
                            <div className="p-4 bg-secondary-500 text-white flex justify-between items-center">
                            ...
                            <div className="flex-1 overflow-y-auto py-2">
                                ...
                                {categories.map(cat => (
                                    <Link key={cat.id} href={`/products?category=${cat.slug}`} onClick={() => setIsMenuOpen(false)} className="block px-6 py-2.5 text-sm text-gray-600 hover:text-primary-600">
                                        {cat.name}
                                    </Link>
                                ))}
                            </div>
                            ...
                            {!isAuthenticated && (
                                <div className="p-4 border-t border-gray-100 flex gap-3">
                                    ...
                                    <Link href="/auth/register" className="flex-1 py-2 text-center bg-secondary-500 text-white rounded text-sm font-medium">Daftar</Link>
                                </div>
                            )}
                        </div>
                    </div>
