...
/**
 * Track shipment by waybill (resi)
 * @param {string} waybill - Receipt number
 * @param {string} courier - Courier code (jne, pos, tiki, jnt, sicepat, etc)
 */
export async function trackWaybill(waybill, courier) {
    try {
        ...
        if (data.rajaongkir.status.code !== 200) {
            // RajaOngkir returns 400 for invalid waybill/courier combination
            // returning null or throwing specific error might be better
