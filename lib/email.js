...
const fromEmail = process.env.RESEND_FROM_EMAIL || 'noreply@infiatin.store';
...
export async function sendVerificationEmail(user, verificationToken) {
    ...
    const subject = 'Verifikasi Email Anda - infiatin.store';
    const html = `
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <style>
                ...
                .content { background: white; padding: 40px; borders: 1px solid #e2e8f0; border-top: none; }
        ...
        <body>
            <div class="container">
                <div class="header">
                    <h1 style="margin: 0;">infiatin.store</h1>
                    <p style="margin: 10px 0 0;">Selamat Datang! 🎉</p>
                </div>
                <div class="content">
                    <h2>Halo ${user.name},</h2>
                    <p>Terima kasih telah mendaftar di <strong>infiatin.store</strong>! Untuk mengaktifkan akun Anda, silakan verifikasi email Anda dengan mengklik tombol di bawah ini:</p>
                    ...
                    <p>Jika Anda tidak mendaftar di infiatin.store, abaikan email ini.</p>
                </div>
                <div class="footer">
                    <p>© ${new Date().getFullYear()} infiatin.store. All rights reserved.</p>
                </div>
            </div>
        </body>
        </html>
...
export async function sendPasswordResetEmail(user, resetToken) {
    ...
    const subject = 'Reset Password - infiatin.store';
    const html = `
        <!DOCTYPE html>
        <html>
        <head>
...
export async function sendOrderConfirmationEmail(order) {
    ...
    return sendEmail({ to: order.users.email, subject, html });
}
...
export async function sendPaymentSuccessEmail(order) {
    const subject = `Pembayaran Berhasil #${order.orderNumber} - infiatin.store`;
    const html = `
        <!DOCTYPE html>
        <html>
        <head>
...
export async function sendOrderCancelledEmail(order) {
    const subject = `Pesanan Dibatalkan #${order.orderNumber} - infiatin.store`;
    const html = `
        <!DOCTYPE html>
        <html>
        <head>
...
export async function sendOrderDeliveredEmail(order) {
    const subject = `Pesanan Diterima #${order.orderNumber} - infiatin.store`;
    const html = `
        <!DOCTYPE html>
        <html>
        <head>
...
export async function sendReviewRequestEmail(order) {
    const subject = `Bagaimana Pengalaman Belanja Anda? - infiatin.store`;
    const html = `
        <!DOCTYPE html>
        <html>
        <head>
...
export async function sendLowStockAlert(products) {
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@infiatin.store';
