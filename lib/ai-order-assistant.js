    ...
    try {
        ...
        const summary = {
            ...
            payments: order.payment ? {
                ...
                paid_at: order.payments.paidAt
            } : null,
            shipping: order.shipment ? {
                courier: order.shipments.courier,
                service: order.shipments.serviceType,
                tracking_number: order.shipments.trackingNumber,
                status: order.shipments.status,
                estimatedDays: order.shipments.estimatedDays,
                shipped_at: order.shipments.shippedAt,
                delivered_at: order.shipments.deliveredAt
            } : null,
...
async function generateOrderInsights(summary) {
    ...
    switch (summary.status) {
        case 'PENDING_PAYMENT':
            ...
            if (summary.payment?.expiresAt && new Date(summary.payments.expiresAt) < new Date()) {
