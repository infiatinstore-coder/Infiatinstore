...
export class FraudDetectionService {
    ...
    async checkOrder(data) {
        ...
        if (!CONFIG.enabled) return { decision: 'ALLOW', riskScore: 0 };
        ...
        if (await this.isBlacklisted('IP', ipAddress)) {
            return { decision: 'BLOCK', riskScore: 100, reason: 'IP blacklisted' };
        }

        if (await this.isBlacklisted('DEVICE', deviceFingerprint)) {
            return { decision: 'BLOCK', riskScore: 100, reason: 'Device blacklisted' };
        }
    ...
    async checkReview(data) {
        ...
        if (!CONFIG.enabled) return { isSuspicious: false, riskScore: 0 };
    ...
    async checkPromoCodeUsage(data) {
        ...
        if (!CONFIG.enabled) return { allowed: true, riskScore: 0 };
