 ...
 * 
 * @returns {{ canTransact: boolean, user: object|null, isGuest: boolean, error?: string }}
 */
export async function assertUserCanTransact(request) {
    ...
    if (!auth.success) {
        return { canTransact: true, user: null, isGuest: true };
    }
    ...
    if (role !== 'USER') {
        ...
        logTransactionGuardBlock(auth.user.id, role, endpoint).catch(err => {
    ...
    return { canTransact: true, user: auth.user, isGuest: false };
}
