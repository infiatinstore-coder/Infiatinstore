...
function CountdownTimer() {
    ...
    return (
        <div className="flex items-center gap-1">
            <div className="bg-white text-[#EE4D2D] px-2 py-1 rounded font-bold text-sm min-w-[30px] text-center shadow-sm">
...
function FlashSaleProductCard({ product }) {
    ...
    const salePrice = product.salePrice || Math.round(product.basePrice * 0.7);
    const discount = Math.round((1 - salePrice / product.basePrice) * 100);
    ...
    return (
        <Link href={`/products/${product.slug}`} className="block bg-white rounded overflow-hidden border border-gray-200/80 hover:border-gray-300 transition-all duration-200 h-full group">
            ...
            <div className="relative aspect-square bg-gray-50">
                ...
                />
                {/* Discount Badge */}
                <div className="absolute top-0 right-0 bg-[#FFD424] text-[#EE4D2D] w-8 h-8 flex flex-col items-center justify-center rounded-bl-lg font-bold leading-none shadow-sm">
            ...
            <div className="p-2">
                <div className="flex flex-col mb-1">
                    <p className="text-sm font-bold text-[#EE4D2D] leading-none mb-0.5">{formatRupiah(salePrice)}</p>
                    <p className="text-[9px] text-gray-400 line-through leading-none">{formatRupiah(product.basePrice)}</p>
                </div>

                {/* Fire Progress Bar */}
                <div className="relative w-full h-3 bg-gray-100 rounded-full overflow-hidden border border-gray-100/50">
                    <div
                        className="absolute left-0 top-0 h-full bg-gradient-to-r from-[#EE4D2D] to-[#ff7e5f]"
...
export default function FlashSaleSection() {
    const displayProducts = mockProducts.slice(0, 6).map((p, i) => ({
        ...p,
        soldCount: 65 + (i * 5),
        salePrice: p.salePrice || Math.round(p.basePrice * 0.8)
    }));

    return (
        <section className="rounded overflow-hidden relative my-3">
            {/* Gradient Background */}
            <div className="absolute inset-0 bg-gradient-to-r from-[#EE4D2D] via-[#ff6b3d] to-[#FF7337] z-0"></div>
