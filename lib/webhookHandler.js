...
import { logPaymentReceived } from './activityLogger';
...
export async function processPaymentWebhook(webhookData) {
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        if (newPaymentStatus) {
            await tx.payments.updateMany({
                ...
                data: {
                    ...
                    ...(newPaymentStatus === 'SUCCESS' && { paid_at: new Date() })
                }
            });
        }
        ...
        // Log payment received to activity log (for audit trail)
        ...
        if (newPaymentStatus === 'SUCCESS') {
            ...
            const fullOrder = await tx.order.findUnique({
                where: { id: order.id },
                include: { users: true, addresses: true, items: true }
            });
