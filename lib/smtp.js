...
export async function sendOrderConfirmationSMTP(order) {
    ...
    if (!recipientEmail) {
        console.warn('⚠️ No email address found for orders:', order.orderNumber);
    ...
    const html = `
        ...
        <head>
            ...
            <style>
                ...
                .content { background: white; padding: 40px; borders: 1px solid #e2e8f0; border-top: none; }
                .tracking-box { background: #eff6ff; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center; borders: 2px dashed #3b82f6; }
