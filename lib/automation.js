...
export async function autoCancelUnpaidOrders() {
    try {
        ...
        for (const order of expiredOrders) {
            ...
            console.log(`❌ Auto-cancelled orders: ${order.orderNumber}`);
            cancelledCount++;
    ...
    try {
        ...
        for (const order of deliveredOrders) {
            ...
            if (order.shipment?.deliveredAt) {
                const deliveredDate = new Date(order.shipments.deliveredAt);
                ...
                if (daysSinceDelivery >= 7) {
                    ...
                    console.log(`✅ Auto-completed orders: ${order.orderNumber}`);
                    completedCount++;
