        ...
        <div className="flex items-center justify-between md:justify-center md:gap-12 px-2 py-2 border-t border-gray-100 mt-2">
            {badges.map((badge, index) => (
                <div key={index} className="flex items-center gap-1.5 text-gray-500">
                    <badge.icon className="w-3.5 h-3.5 text-[#EE4D2D]" />
...
function ProductGridSection({ title, subtitle, products = [], showViewAll = true, bg = "bg-transparent" }) {
            ...
            <div className={`flex items-end justify-between mb-4 ${bg === 'bg-white' ? 'px-3 pt-3 md:px-4 md:pt-4' : 'px-1'}`}>
                <div>
                    <h2 className="text-base md:text-xl font-bold text-gray-800 uppercase tracking-tight flex items-center gap-2">
                        <span className="w-1.5 h-6 bg-[#EE4D2D] rounded-full block"></span>
                        {title}
                    </h2>
                    {subtitle && <p className="text-xs text-gray-500 mt-1 pl-4 hidden md:block">{subtitle}</p>}
                </div>
                {showViewAll && (
                    <Link href="/products" className="flex items-center gap-1 text-xs font-bold text-[#EE4D2D] hover:underline uppercase mr-1">
                        Lihat Semua
                ...
                <div className="container-app mb-4 px-2 md:px-0">
                    <div className="bg-white rounded border border-gray-100 p-3 md:p-5">
                        <div className="flex items-center justify-between mb-4 border-b border-gray-50 pb-3">
                            ...
                            <Link href="/categories" className="text-xs font-bold text-[#EE4D2D] hover:underline">Lihat Semua</Link>
                        </div>
                        <CategoryIconGrid />
                    </div>
                </div>
                ...
                */}
                <div className="container-app space-y-4 px-2 md:px-0 pb-10">
                    ...
                    <div className="flex justify-center mt-4">
                        <Link href="/products">
                            <button className="bg-white text-gray-700 border border-gray-300 hover:border-[#EE4D2D] hover:text-[#EE4D2D] px-8 py-2 md:px-16 md:py-3 rounded-full font-semibold transition-all shadow-sm text-sm tracking-wide hover:shadow-md">
                                Muat Lebih Banyak
                            </button>
                        </Link>
                    </div>
